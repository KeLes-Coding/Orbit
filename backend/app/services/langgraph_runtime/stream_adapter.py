...
call_model 节点通过 get_stream_writer() 发射的自定义事件，
经此适配器转换为 conversation_stream_store 中的标准 SSE 事件，
同时累积响应文本、推理文本、token 用量等状态。
"""
...
class StreamAdapter:
    ...
    职责：
    1. 将 call_model 节点的自定义流事件写入 conversation_stream_store
    2. 累积 response_text, reasoning_text, token_usage, response_metadata
    ...
    def get_accumulated_state(self) -> dict[str, Any]:
        ...
        在 call_model 节点返回时调用，将累积的流式输出转为 graph state 更新。
        """
